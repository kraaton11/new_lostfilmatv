...
class TrustedDeviceService:
    def __init__(
        ...
        *,
        ...
        self._cookie_domain = cookie_domain.strip().lstrip(".").rstrip(".") if cookie_domain else None
    ...
    def resolve(self, request: Request) -> TrustedDeviceSession | None:
        if not self.enabled:
            return None
        ...
        if session is None:
            return None
        if not self._verify_lostfilm_session(session.payload):
            self.revoke_token(token)
            return None
        return session

    def remember(self, response: Response, payload: SessionPayload, *, previous_token: str | None = None) -> None:
        if not self.enabled:
            return
        ...
        if previous_token:
            self.revoke_token(previous_token)
        ...
        logger.info("Trusted device remembered: device_id=%s account_id=%s", mask_token(device_id), mask_token(payload.accountId))
    ...
    def prune_expired(self) -> None:
        ...
        with self._lock, self._connection() as connection:
            connection.execute("DELETE FROM trusted_devices WHERE expires_at <= ? OR revoked_at IS NOT NULL", (now,))
    ...
    def _load_session(self, token: str) -> TrustedDeviceSession | None:
        now = datetime.now(UTC)
        with self._lock, self._connection() as connection:
            row = connection.execute(
                """
                SELECT device_id, encrypted_payload
                FROM trusted_devices
                WHERE token_hash = ? AND revoked_at IS NULL AND expires_at > ?
                """,
                (self._hash_token(token), _format_dt(now)),
            ).fetchone()
            if row is None:
                return None
            connection.execute(
        ...
        try:
            ...
            self.revoke_token(token)
            return None
        return TrustedDeviceSession(token=token, device_id=row["device_id"], payload=payload)

    def _verify_lostfilm_session(self, payload: SessionPayload) -> bool:
        ...
        try:
            with httpx.Client(
                ...
                response = client.get(f"{self._lostfilm_base_url}/")
    ...
    def _ensure_schema(self) -> None:
        ...
        with self._lock, self._connection() as connection:
            ...
            connection.execute("CREATE INDEX IF NOT EXISTS idx_trusted_devices_expires_at ON trusted_devices(expires_at)")
